...
from fusionsupervision.log import setup_logger, ALIGNAK_LOGGER_NAME, ColorStreamHandler, CollectorHandler
...
from fusionsupervision.scheduler import Scheduler
from fusionsupervision.macroresolver import MacroResolver
from fusionsupervision.external_command import ExternalCommandManager, ExternalCommand
...
class FusionsupervisionTest(unittest2.TestCase):
    ...
    def setUp(self):
        ...
        logger_configuration_file = os.path.join(os.getcwd(), './etc/fusionsupervision-logger.json')
        ...
        for daemon in ['broker', 'poller', 'reactionner', 'receiver', 'scheduler', 'arbiter']:
            for proc in psutil.process_iter():
                if 'fusionsupervision' in proc.name() and daemon in proc.name():
                    running_daemons = True
        if running_daemons:
            self._stop_fusionsupervision_daemons(arbiter_only=False)
...
"""
    ...
    def _prepare_configuration(self, copy=True, cfg_folder='/tmp/fusionsupervision', daemons_list=None):
        ...
        for f in ['fusionsupervision.log', 'fusionsupervision-events.log']:
        ...
        files = ['%s/etc/fusionsupervision.ini' % cfg_folder,
                 '%s/etc/fusionsupervision.d/daemons.ini' % cfg_folder,
                 '%s/etc/fusionsupervision.d/modules.ini' % cfg_folder]
        ...
        try:
            ...
            # Nagios legacy files
            cfg.set('fusionsupervision-configuration', 'cfg', '%s/etc/fusionsupervision.cfg' % cfg_folder)

            # Daemons launching and check
            cfg.set('fusionsupervision-configuration', 'polling_interval', '1')
            cfg.set('fusionsupervision-configuration', 'daemons_check_period', '1')
            cfg.set('fusionsupervision-configuration', 'daemons_stop_timeout', '10')
            cfg.set('fusionsupervision-configuration', 'daemons_start_timeout', '1')
            cfg.set('fusionsupervision-configuration', 'daemons_new_conf_timeout', '1')
            cfg.set('fusionsupervision-configuration', 'daemons_dispatch_timeout', '1')

            # Poller/reactionner workers count limited to 1
            cfg.set('fusionsupervision-configuration', 'min_workers', '1')
            cfg.set('fusionsupervision-configuration', 'max_workers', '1')

            with open('%s/etc/fusionsupervision.ini' % cfg_folder, "w") as modified:
                cfg.write(modified)
    ...
    def _stop_fusionsupervision_daemons(self, arbiter_only=True, request_stop_uri=''):
        ...
        If some fusionsupervision- daemons are still running after the kill, force kill them.
    ...
    def _run_fusionsupervision_daemons(self, cfg_folder='/tmp/fusionsupervision', runtime=30,
        ...
        if update_configuration:
            ...
            try:
                ...
                cfg.set('fusionsupervision-configuration', 'polling_interval', '1')
                cfg.set('fusionsupervision-configuration', 'daemons_check_period', '1')
                cfg.set('fusionsupervision-configuration', 'daemons_stop_timeout', '20')
                cfg.set('fusionsupervision-configuration', 'daemons_start_timeout', '5')
                cfg.set('fusionsupervision-configuration', 'daemons_new_conf_timeout', '1')
                cfg.set('fusionsupervision-configuration', 'daemons_dispatch_timeout', '1')
        ...
        self._stop_fusionsupervision_daemons()
        ...
        for name in daemons_list + ['arbiter-master']:
            if arbiter_only and name not in ['arbiter-master']:
                continue
            args = ["../fusionsupervision/bin/fusionsupervision_%s.py" % name.split('-')[0], "-n", name,
                    "-e", "%s/etc/fusionsupervision.ini" % cfg_folder]
            if verbose:
                args.append("--debug")
        ...
        for name, proc in list(self.procs.items()):
            ...
            if ret is not None:
                ...
                if os.path.exists("/tmp/fusionsupervision.log"):
                    with open("/tmp/fusionsupervision.log") as f:
                        for line in f:
                            print("xxx %s" % line)
    ...
    def _check_daemons_log_for_errors(self, daemons_list, run_folder='/tmp/fusionsupervision',
    ...
    def setup_with_file(self, configuration_file=None, env_file=None,
        ...
        """
        Load fusionsupervision with the provided configuration and environment files
        ...
        :param env_file: path + file name of the fusionsupervision environment file
        :type env_file: str
        ...
        #     cfg_folder = '/tmp/etc/fusionsupervision'
        #     files = ['%s/fusionsupervision.ini' % cfg_folder,
        #              '%s/fusionsupervision.d/daemons.ini' % cfg_folder,
        #              '%s/fusionsupervision.d/modules.ini' % cfg_folder,
        #              '%s/fusionsupervision-logger.json' % cfg_folder]
        #     replacements = {
        #         '_dist=/usr/local/': '_dist=/tmp',
        #         'user=fusionsupervision': ';user=fusionsupervision',
        #         'group=fusionsupervision': ';group=fusionsupervision'
        #
        #     }
        ...
        if env_file is not None:
            ...
            self.env_filename = os.path.join(configuration_dir, 'fusionsupervision.ini')
            if os.path.exists(os.path.join(configuration_dir, 'fusionsupervision.ini')):
                # fusionsupervision.ini in the same directory as the legacy configuration file
                self.env_filename = os.path.join(configuration_dir, 'fusionsupervision.ini')
            elif os.path.exists(os.path.join(current_dir, './etc/fusionsupervision.ini')):
                # fusionsupervision.ini in the test/etc directory
                self.env_filename = os.path.join(current_dir, './etc/fusionsupervision.ini')
            else:
        ...
        arbiter_cfg = None
        for daemon_section, daemon_cfg in list(self.fusionsupervision_env.get_daemons().items()):
        ...
        args = {
            'fusionsupervision_name': 'fusionsupervision-test', 'daemon_name': arbiter_name,
        ...
        try:
            ...
            self._arbiter.setup_fusionsupervision_logger()
