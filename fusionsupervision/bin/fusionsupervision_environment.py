...
alignak-environment command line interface::
    ...
    Output:
        ...
        As an example for a file as the default ./etc/alignak.ini, the script will output:
            ...
            export ALIGNAK_CONFIGURATION_CONFIG_NAME='FusionSupervision Engine global configuration'
            export ALIGNAK_CONFIGURATION_ALIGNAK_NAME='My FusionSupervision'
            ...
            export DAEMON_ARBITER_MASTER_CONFIG_NAME='FusionSupervision Engine global configuration'
            export DAEMON_ARBITER_MASTER_ALIGNAK_NAME='My FusionSupervision'
        ...
        NOTE: this script also adds the current FusionSupervision Engine version to the content of the
        configuration file
    ...
    Use cases:
        ...
        Parse FusionSupervision Engine configuration file and define environment variables
            cfg_file ../etc/alignak-realm2.ini

        Parse FusionSupervision Engine configuration file and define environment variables and print information
...
from fusionsupervision.version import VERSION as __version__

SECTION_CONFIGURATION = "fusionsupervision-configuration"


class FusionsupervisionConfigParser(object):
    """
    Class to parse the FusionSupervision Engine main configuration file
    """
    ...
    def __init__(self, args=None):
        ...
        When used without args, it is considered as called by the FusionSupervision engine daemon creation
        ...
        :param args:
        """
        # FusionSupervision Engine version as a property
        self.fusionsupervision_version = __version__
        ...
        if '--verbose' in args and args['--verbose']:
            print("FusionSupervision Engine environment parser:")
    ...
    def parse(self):
        ...
        """
        Check if some extra configuration files are existing in an `fusionsupervision.d` sub directory
        near the found configuration file.

        Parse the FusionSupervision Engine configuration file(s)

        Exit the script if some errors are encountered.

        :return: True/False
        """
        # Search if some ini files existe in an fusionsupervision.d sub-directory
        sub_directory = 'fusionsupervision.d'
    ...
    def write(self, env_file):
        """
        Write the FusionSupervision Engine configuration to a file
    ...
    def get_legacy_cfg_files(self):
        """
        Get the FusionSupervision Engine monitored configuration files.

        :return: a dict containing the FusionSupervision Engine legacy configuration files
        """
        return self.get_fusionsupervision_configuration(legacy_cfg=True)

    def get_fusionsupervision_macros(self):
        """
        Get the FusionSupervision Engine macros.

        :return: a dict containing the FusionSupervision Engine macros
        """
        macros = self.get_fusionsupervision_configuration(macros=True)

        sections = self._search_sections('pack.')
        for name, _ in list(sections.items()):
            section_macros = self.get_fusionsupervision_configuration(section=name, macros=True)
            macros.update(section_macros)
        return macros

    def get_fusionsupervision_configuration(self, section=SECTION_CONFIGURATION,
                                  legacy_cfg=False, macros=False):
        """
        Get the FusionSupervision Engine configuration parameters. All the variables included in
        ...
        :type macros: bool
        :return: a dict containing the FusionSupervision Engine configuration parameters
        """
...
def main():
    ...
    Main function
    """
    parsed_configuration = FusionsupervisionConfigParser()
    try:
    ...
    if parsed_configuration.export:
        # Export FusionSupervision Engine version
        print("export FUSIONSUPERVISION_VERSION=%s" % (parsed_configuration.fusionsupervision_version))
