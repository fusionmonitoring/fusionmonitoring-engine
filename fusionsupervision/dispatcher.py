...
"""
 This is the Dispatcher class. Its role is to prepare the FusionSupervision Engine configuration to get
 dispatched to the FusionSupervision Engine satellites like schedulers, reactionners, pollers, receivers
...
from fusionsupervision.util import master_then_spare
from fusionsupervision.objects.satellitelink import LinkError
...
class Dispatcher(object):
    ...
    def __init__(self, conf, arbiter_link):
        ...
        :param conf: the whole FusionSupervision Engine configuration as parsed by the Arbiter
        :type conf: Config
        ...
        self.fusionsupervision_conf = conf
        self.global_conf = {}
        # Get configuration data from the pushed configuration
        cls = self.fusionsupervision_conf.__class__
        for prop, entry in list(cls.properties.items()):
            # Is this property intended for broking?
            if 'full_status' not in entry.fill_brok:
                continue
            self.global_conf[prop] = self.fusionsupervision_conf.get_property_value_for_brok(
                prop, cls.properties)
            # self.global_conf[prop] = getattr(self.fusionsupervision_conf, prop, entry.default)
        logger.debug("Dispatcher configuration: %s / %s", self.arbiter_link, self.fusionsupervision_conf)
        logger.debug("Dispatcher global configuration: %s", self.global_conf)

        logger.info("Dispatcher realms configuration:")
        for realm in self.fusionsupervision_conf.realms:
        ...
        for sat_type in ['arbiters', 'schedulers', 'reactionners',
            ...
            setattr(self, sat_type, getattr(self.fusionsupervision_conf, sat_type))
    ...
    def get_satellites_list(self, sat_type):
        ...
        :rtype: list[fusionsupervision.objects.satellitelink.SatelliteLink]
        """
        satellites_list = []
    ...
    def get_scheduler_ordered_list(self, realm):
        ...
        :type realm: fusionsupervision.objects.realm.Realm
        :return: sorted scheduler list
        :rtype: list[fusionsupervision.objects.schedulerlink.SchedulerLink]
        """
    ...
    def prepare_dispatch(self):
        ...
        # Update FusionSupervision Engine name for all the satellites
        for daemon_link in self.all_daemons_links:
            daemon_link.cfg.update({'fusionsupervision_name': self.fusionsupervision_conf.fusionsupervision_name})
        ...
        for arbiter_link in self.get_satellites_list('arbiters'):
            ...
            arbiter_cfg.update({
                'managed_hosts_names': [h.get_name() for h in self.fusionsupervision_conf.hosts],
                'modules': serialize(arbiter_link.modules, True),

                'managed_conf_id': self.fusionsupervision_conf.instance_id,
                'push_flavor': ''
            })
            ...
            if arbiter_link != self.arbiter_link:
                arbiter_cfg.update({
                    ...
                    'whole_conf': self.fusionsupervision_conf.spare_arbiter_conf,
                })
            ...
            arbiter_link.cfg_to_manage = self.fusionsupervision_conf
        ...
        # main_realm = self.fusionsupervision_conf.realms.find_by_name('All')
        # all_realms = main_realm.all_sub_members
        # for realm_uuid in all_realms:
        #     realm = self.fusionsupervision_conf.realms[realm_uuid]
                    ...
                    for sat_link in satellites:
                        ...
                        if sat_type == "broker":
                            sat_link.cfg.update({'satellites': realm.get_links_for_a_broker(
                                ...
                                self.fusionsupervision_conf.realms, sat_link.manage_sub_realms)})
        ...
        nb_missed = len([cfg for cfg in list(
            self.fusionsupervision_conf.parts.values()) if not cfg.is_assigned])
        if nb_missed > 0:
