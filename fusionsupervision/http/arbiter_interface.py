...
from fusionsupervision.http.generic_interface import GenericInterface
from fusionsupervision.util import split_semicolon
...
class ArbiterInterface(GenericInterface):
    ...
    def reload_configuration(self):
        ...
        **Note** tha the arbiter will not reload its main configuration file (eg. fusionsupervision.ini)
        but it will reload the monitored objects from the Nagios legacy files or from the
        FusionSupervision backend!
    ...
    def backend_notification(self, event=None, parameters=None):
        """The FusionSupervision Engine raises an event to the FusionSupervision Engine arbiter
        -----
        Possible events are:
    ...
    def monitoring_problems(self):
        """Get FusionSupervision Engine detailed monitoring status
    ...
    def livesynthesis(self):
        """Get FusionSupervision live synthesis
    ...
    def object(self, o_type, o_name=None):
        ...
        The FusionSupervision Engine unserialize function of the fusionsupervision.misc.serialization package allows
        ...
        As an example:
        {
            "__sys_python_module__": "fusionsupervision.objects.hostgroup.Hostgroup",
            "content": {
    ...
    def status(self, details=False):
        """Get the overall fusionsupervision status
        ...
        Returns a list of the satellites as in:
        {
            ...
            name: "My FusionSupervision",
            template: {
                notes: "",
                alias: "My FusionSupervision",
                _templates: [
                    "fusionsupervision",
                    "important"
                ],
        ...
        return self.app.get_fusionsupervision_status(details=details)

    @cherrypy.expose
    @cherrypy.tools.json_out()
    def events_log(self, details=False, count=0, timestamp=0):
        """Get the most recent FusionSupervision Engine events
        ...
        The arbiter maintains a list of the most recent FusionSupervision Engine events. This endpoint
        provides this list.
    ...
    def host(self):
        ...
        # Check if the host exist in FusionSupervision Engine
        # ---
            ...
            try:
                ...
                if timestamp < last_ts:
                    logger.info("Got unordered timestamp for the host '%s'. "
                                "The FusionSupervision Engine scheduler may not handle the check result!",
                                host_name)
                last_ts = timestamp
            except ValueError:
                pass
    ...
    def _push_external_command(self, command=None):
        ...
        *command* endpoint.
        ...
        :param command: FusionSupervision Engine external command
        :type command: string
        :return: None
        """
