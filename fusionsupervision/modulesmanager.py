...
"""
This module provides ModulesManager class. Used to load modules in FusionSupervision Engine
"""
...
class ModulesManager(object):
    ...
    def get_instances(self):
        ...
        for (fusionsupervision_module, python_module) in self.modules_assoc:
            fusionsupervision_module.properties = python_module.properties.copy()
            fusionsupervision_module.my_daemon = self.daemon
            logger.info("FusionSupervision Engine starting module '%s'", fusionsupervision_module.get_name())
            if getattr(fusionsupervision_module, 'modules', None):
                modules = []
                for module_uuid in fusionsupervision_module.modules:
                    if module_uuid in self.modules:
                        modules.append(self.modules[module_uuid])
                fusionsupervision_module.modules = modules
            logger.debug("Module '%s', parameters: %s",
                         fusionsupervision_module.get_name(), fusionsupervision_module.__dict__)
            try:
                instance = python_module.get_instance(fusionsupervision_module)
                if not isinstance(instance, BaseModule):  # pragma: no cover, simple protection
                    self.configuration_errors.append("Module %s instance is not a "
                                                     ...
                                                     % (fusionsupervision_module.get_name(),
                                                        type(instance)))
                    raise AttributeError
            # pragma: no cover, simple protection
            except Exception as exp:  # pylint: disable=broad-except
                ...
                self.configuration_errors.append("The module %s raised an exception on "
                                                 ...
                                                 % (fusionsupervision_module.get_name(), str(exp)))
            else:
