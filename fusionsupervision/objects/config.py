...
"""
 Config is the class that reads, loads and manipulates the main FusionSupervision Engine monitored objects
...
from fusionsupervision.fusionsupervisionobject import get_a_new_object_id
from fusionsupervision.misc.serialization import serialize

from fusionsupervision.commandcall import CommandCall
from fusionsupervision.objects.item import Item
...
from fusionsupervision.objects.checkmodulation import CheckModulation, CheckModulations
...
from fusionsupervision.objects.hostextinfo import HostExtInfo, HostsExtInfo
from fusionsupervision.objects.serviceextinfo import ServiceExtInfo, ServicesExtInfo
# from fusionsupervision.objects.trigger import Trigger, Triggers
# from fusionsupervision.objects.pack import Packs
from fusionsupervision.util import split_semicolon, to_name_if_possible
from fusionsupervision.objects.arbiterlink import ArbiterLink, ArbiterLinks
from fusionsupervision.objects.schedulerlink import SchedulerLink, SchedulerLinks
from fusionsupervision.objects.reactionnerlink import ReactionnerLink, ReactionnerLinks
from fusionsupervision.objects.brokerlink import BrokerLink, BrokerLinks
from fusionsupervision.objects.receiverlink import ReceiverLink, ReceiverLinks
from fusionsupervision.objects.pollerlink import PollerLink, PollerLinks
from fusionsupervision.graph import Graph
from fusionsupervision.property import (UnusedProp, BoolProp, IntegerProp, CharProp,
                              StringProp, ListProp, ToGuessProp)
from fusionsupervision.util import jsonify_r


logger = logging.getLogger(__name__)  # pylint: disable=invalid-name

NO_LONGER_USED = (u'This parameter is not longer take from the main file, but must be defined '
                  u'in the status_dat broker module instead. But FusionSupervision Engine will create you one '
...
NOT_MANAGED = (u'This Nagios legacy parameter is not managed by FusionSupervision Engine. Ignoring...')
...
class Config(Item):  # pylint: disable=R0904,R0902
 ...
 dispatches to the other daemons."""
    ...
    properties = {
        ...
        # FusionSupervision Engine instance name is set as the arbiter name
        ...
        'command_check_interval':
            UnusedProp(text=u'FusionSupervision Engine will always check for external commands. '
                            u'This configuration value is useless.'),
        'command_file':
            StringProp(default=u''),
        'external_command_buffer_slots':
            UnusedProp(text=u'FusionSupervision Engine do not limit the external commands slot.'),
        ...
        'check_for_updates':
            UnusedProp(text=u'network administrators will never allow such communication between '
                            ...
                            u'http://www.github.com/fusionsupervision/fusionsupervision-engine website instead.'),
        ...
        # Monitoring logs (FusionSupervision Engine events log) configuration
        'events_date_format':
        ...
        'sleep_time':
            UnusedProp(text=u'This deprecated option is useless in the fusionsupervision way of doing.'),

        'service_inter_check_delay_method':
            UnusedProp(text=u'This option is useless in the FusionSupervision Engine scheduling. '
        ...
        'check_result_reaper_frequency':
            UnusedProp(text=u'FusionSupervision Engine do not use reaper process.'),

        'max_check_result_reaper_time':
            UnusedProp(text=u'FusionSupervision Engine do not use reaper process.'),

        'check_result_path':
            UnusedProp(text=u'FusionSupervision Engine use in memory returns, not check results on flat file.'),

        'max_check_result_file_age':
            UnusedProp(text=u'FusionSupervision Engine do not use flat file check resultfiles.'),

        'host_inter_check_delay_method':
            UnusedProp(text=u'This option is unused in the FusionSupervision Engine scheduling because distribution '
        ...
        'translate_passive_host_checks':
            UnusedProp(text=u'FusionSupervision Engine passive checks management makes this parameter unuseful.'),
        ...
        'use_large_installation_tweaks':
            UnusedProp(text=u'this option is deprecated because in fusionsupervision it is just an alias '
        ...
        'host_perfdata_file_template':
            StringProp(managed=False, default='/tmp/host.perf',
                       _help='Smartly replaced with the FusionSupervision Engine inner metrics feature or backend.'),

        'service_perfdata_file_template':
            StringProp(managed=False, default='/tmp/host.perf',
                       _help='Smartly replaced with the FusionSupervision Engine '
        ...
        'daemons_log_folder':
            StringProp(default='/usr/local/var/log/fusionsupervision',
    ...
    old_properties = {
        'nagios_user':  'fusionsupervision_user',
        'nagios_group': 'fusionsupervision_group'
    }
    ...
    def load_params(self, params):
        ...
        :return: None
        """
        logger.debug("FusionSupervision Engine parameters:")
    ...
    def read_legacy_cfg_files(self, cfg_files, fusionsupervision_env_files=None):
        ...
        :type cfg_files: list
        :param fusionsupervision_env_files: name of the fusionsupervision environment file
        :type fusionsupervision_env_files: list
        ...
        self.fusionsupervision_env = 'n/a'
        if fusionsupervision_env_files is not None:
            self.fusionsupervision_env = fusionsupervision_env_files
            if not isinstance(fusionsupervision_env_files, list):
                self.fusionsupervision_env = [os.path.abspath(fusionsupervision_env_files)]
            else:
                self.fusionsupervision_env = [os.path.abspath(f) for f in fusionsupervision_env_files]
    ...
    def read_config_buf(self, cfg_buffer):
        ...
        dictionary containing dictionary items tha tmay be used to create FusionSupervision Engine
        objects

        :param cfg_buffer: buffer containing all data from config files
        :type cfg_buffer: str
        :return: dict of fusionsupervision objects with the following structure ::
        { type1 : [{key: value, ..}, {..}],
          type2 : [ ... ]
        }
    ...
    def add_self_defined_objects(raw_objects):
        ...
        raw_objects['command'].append({
            ...
            'imported_from': 'fusionsupervision-self'
        })
    ...
    def create_objects_for_type(self, raw_objects, o_type):
        ...
        This function create real FusionSupervision Engine objects from the raw data got from the configuration.
    ...
    def warn_about_unmanaged_parameters(self):
        ...
        if unmanaged:
            logger.warning("The following Nagios legacy parameter(s) are not currently "
                           "managed by FusionSupervision Engine:")

            for line in unmanaged:
                logger.warning('- %s', line)

            logger.warning("Those are unmanaged configuration statements, do you really need it? "
                           "Create an issue on the FusionSupervision Engine repository or submit a pull "
                           "request: http://www.github.com/fusionsupervision/fusionsupervision-engine")
    ...
    def log_daemons_list(self):
        """Log FusionSupervision Engine daemons list
    ...
    def fill_default_satellites(self, fusionsupervision_launched=False):
        ...
        :param fusionsupervision_launched: created daemons are to be launched or not
        :type fusionsupervision_launched: bool
        :return: None
        """

        # Log all satellites list
        logger.debug("FusionSupervision Engine configured daemons list:")
        ...
        if not self.schedulers:
            ...
            satellite = SchedulerLink({'type': 'scheduler', 'name': 'Default-Scheduler',
                                       ...
                                       'fusionsupervision_launched': fusionsupervision_launched,
                                       'missing_daemon': True,
        ...
        for satellites_list in [self.pollers, self.brokers, self.reactionners,
            ...
            if not hosts_realms_names.issubset(sat_realms_names):
                ...
                for realm_name in hosts_realms_names.difference(sat_realms_names):
                    ...
                    if not fusionsupervision_launched:
                        continue
                    ...
                    new_daemon = sat_class({
                        ...
                        'fusionsupervision_launched': True, 'missing_daemon': True,
        ...
        # Log all satellites list
        logger.debug("FusionSupervision Engine definitive daemons list:")
        self.log_daemons_list()
    ...
    def hack_old_nagios_parameters(self):
        ...
        if getattr(self, 'service_perfdata_file', None) or \
            ...
            msg = "The configuration parameters '%s = %s' and '%s = %s' are Nagios legacy " \
                  "parameters. FusionSupervision Engine will use its inner 'metrics' module " \
        ...
        if hasattr(self, 'retain_state_information') and self.retain_state_information:
            ...
            msg = "The configuration parameter '%s = %s' is a Nagios legacy " \
                  "parameter. FusionSupervision Engine will use its inner 'retention' module " \
                  "to match the expected behavior." \
    ...
    def is_correct(self):  # pylint: disable=too-many-branches, too-many-statements, too-many-locals
        ...
        # Check if fusionsupervision_name is defined
        if not self.fusionsupervision_name:
            logger.info('FusionSupervision Engine name is not defined, using the main arbiter name...')
            for arbiter in self.arbiters:
                if not arbiter.spare:
                    self.fusionsupervision_name = arbiter.name
                    break
        logger.info('FusionSupervision Engine name is: %s', self.fusionsupervision_name)
    ...
    def dump(self, dump_file_name=None):
        ...
        if not dump_file_name:
            dump_file_name = os.path.join(tempfile.gettempdir(),
                                          'fusionsupervision-%s-cfg-dump-%d.json'
