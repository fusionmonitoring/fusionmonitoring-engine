...
"""
This module allows to export FusionSupervision Engine internal metrics to a statsd server.

The `register` function allows an FusionSupervision Engine daemon to register some metrics and the
expected behavior (sends to StatsD server and/or build an internal brok).

The `connect` function allows an FusionSupervision Engine daemon to connectto a Carbon/Graphite server.
...
if sys.version_info >= (2, 7):
    from fusionsupervision.misc.carboniface import CarbonIface
...
class Stats(object):
    ...
    def register(self, name, _type, statsd_host='localhost', statsd_port=8125,
                 statsd_prefix='fusionsupervision', statsd_enabled=False, broks_enabled=False):
        ...
        if self.statsd_enabled and self.statsd_host is not None and self.statsd_host != 'None':
            ...
            if self.load_statsd():
                logger.info('FusionSupervision Engine internal statistics are sent to StatsD.')
            else:
        ...
        if self.stats_file:
            try:
                ...
                logger.info("FusionSupervision Engine internal statistics are written in the file %s",
                            self.stats_file)
    ...
    def connect(self, name, _type, host='localhost', port=2004,
                prefix='fusionsupervision', enabled=False, broks_enabled=False):
        ...
        if self.statsd_enabled and self.statsd_host is not None and self.statsd_host != 'None':
            ...
            logger.info('FusionSupervision Engine internal statistics will be sent to Graphite.')
    ...
    def timer(self, key, value, timestamp=None):
        ...
        if self.broks_enabled:
            ...
            return Brok({'type': 'fusionsupervision_stat',
                         'data': {
                             'ts': timestamp,
                             'type': 'timer',
