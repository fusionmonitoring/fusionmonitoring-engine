...
import re
import logging
from fusionsupervision.util import to_float, to_split, to_char, to_int, unique_value, list_split
...
class Property(object):
    ...
    def __init__(self, default=NONE_OBJECT, class_inherit=None,  # pylint: disable=R0913
        ...
        managed: property that is managed in Nagios but not in FusionSupervision Engine
...
class UnusedProp(Property):
    ...
    no longer useful/used by FusionSupervision Engine.
       ...
       'text' a some usage text if present, will print it to explain
        ...
        if text is None:
            text = ("This parameter is no longer useful in the "
                    "FusionSupervision Engine architecture.")
        self.text = text
        self.unused = True
