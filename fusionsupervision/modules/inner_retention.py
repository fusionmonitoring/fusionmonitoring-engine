...
from fusionsupervision.stats import Stats
...
class InnerRetention(BaseModule):
    ...
    def __init__(self, mod_conf):  # pylint: disable=too-many-branches
        ...
        stats_prefix = getattr(mod_conf, 'statsd_prefix', 'fusionsupervision')
        ...
        if self.retention_file is None:
            self.retention_file = os.path.join(self.retention_dir, 'fusionsupervision-retention-%s.json')
    ...
    def hook_load_retention(self, scheduler):  # pylint: disable=too-many-locals, too-many-branches
        ...
        :param scheduler: scheduler instance of fusionsupervision
        :type scheduler: object
        :return: None
        """
        if not self.enabled:
            logger.warning("FusionSupervision Engine retention module is not enabled."
    ...
    def hook_save_retention(self, scheduler):
        ...
        try:
            ...
            if not data_to_save:
                logger.warning("FusionSupervision Engine retention data to save are not containing any information.")
                return None

            # Move services data to their respective hosts dictionary
            # FusionSupervision Engine scheduler do not merge the services into the host dictionary!
