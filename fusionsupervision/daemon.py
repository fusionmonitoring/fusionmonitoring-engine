...
"""
This module provides abstraction for creating daemon in FusionSupervision Engine
"""
...
from fusionsupervision.log import setup_logger, set_log_level
from fusionsupervision.http.daemon import HTTPDaemon, PortNotFree
from fusionsupervision.stats import statsmgr
from fusionsupervision.modulesmanager import ModulesManager
from fusionsupervision.property import StringProp, BoolProp, PathProp
from fusionsupervision.property import IntegerProp, FloatProp, LogLevelProp, ListProp
from fusionsupervision.misc.common import setproctitle, SIGNALS_TO_NAMES_DICT
from fusionsupervision.version import VERSION

from fusionsupervision.bin.fusionsupervision_environment import FusionsupervisionConfigParser


logger = logging.getLogger(__name__)  # pylint: disable=invalid-name

# #########################   DAEMON PART    ###############################
# Recommended (default) umask for a daemonized process is 0. Former FusionSupervision Engine one was 027...
# UMASK = 027
UMASK = 0
...
class EnvironmentFile(Exception):
    """Exception raised when the FusionSupervision Engine environment file is missing or corrupted"""
...
class Daemon(object):
    """Class providing daemon level call for FusionSupervision Engine
    """
    ...
    properties = {
        ...
        'name':
            StringProp(),
        # FusionSupervision Engine main configuration file
        'env_filename':
        ...
        # Daemon directories
        'etcdir':   # /usr/local/etc/fusionsupervision
            PathProp(default=DEFAULT_WORK_DIR),
        'workdir':  # /usr/local/var/run/fusionsupervision
            PathProp(default=DEFAULT_WORK_DIR),
        'vardir':   # /usr/local/var/lib/fusionsupervision
            PathProp(default=DEFAULT_WORK_DIR),
        'logdir':   # /usr/local/var/log/fusionsupervision
            PathProp(default=DEFAULT_WORK_DIR),
        ...
        'logger_configuration':
            StringProp(default=u'./fusionsupervision-logger.json'),
        ...
        # FusionSupervision Engine will report its status to its monitor
        # Interface is the same as the FusionSupervision Engine WS module PATCH/host
        'fusionsupervision_monitor':
            StringProp(default=u''),
        'fusionsupervision_monitor_period':
            IntegerProp(default=60),
        'fusionsupervision_monitor_username':
            StringProp(default=u''),
        'fusionsupervision_monitor_password':
            StringProp(default=u''),
    ...
    def __init__(self, name, **kwargs):
        ...
        """
        self.fusionsupervision_env = None
        ...
        if os.environ.get('ALIGNAK_CONFIGURATION_FILE'):
            ...
            print("FusionSupervision Engine environment file from system environment: %s" % kwargs['env_file'])

        # I must have an FusionSupervision Engine environment file
        if 'env_file' not in kwargs:
            self.exit_on_error("FusionSupervision Engine environment file is missing or corrupted", exit_code=1)
        ...
        if kwargs['env_file']:
            ...
            # Read FusionSupervision Engine environment file
            args = {'<cfg_file>': self.env_filename, '--verbose': self.debug}
            configuration_dir = os.path.dirname(self.env_filename)
            try:
                self.fusionsupervision_env = FusionsupervisionConfigParser(args)
                self.fusionsupervision_env.parse()

                legacy_cfg_files = self.fusionsupervision_env.get_legacy_cfg_files()
                if legacy_cfg_files:
                    for prop, value in list(self.fusionsupervision_env.get_legacy_cfg_files().items()):
                        self.pre_log.append(("DEBUG",
                                             "Found FusionSupervision Engine monitoring "
                ...
                if self.type == 'arbiter' and not self.legacy_cfg_files:
                    ...
                    self.pre_log.append(("WARNING",
                                         "If you need some, edit the 'fusionsupervision.ini' configuration "
                                         "file to declare one or more 'cfg=' variables."))

                my_configuration = list(self.fusionsupervision_env.get_daemons(daemon_name=self.name).items())
                ...
                if not my_configuration:
                    ...
                    # self.pre_log.append(("DEBUG",
                    #                      "No defined configuration for the daemon: %s. "
                    #                      "Using the 'fusionsupervision-configuration' section "
                    #                      "variables as parameters for the daemon:" % self.name))
                    #
                    # # Set the global FusionSupervision Engine configuration parameters
                    # # as the current daemon properties
                    # self.pre_log.append(("INFO",
                    #                      "Get fusionsupervision configuration to configure the daemon..."))
                    ...
                    #     for prop, value in list(fusionsupervision_configuration.items()):
                ...
                print("Daemon '%s' did not correctly read FusionSupervision Engine environment file: %s"
        ...
        if not self.active:
            self.exit_ok(message="This daemon is disabled in FusionSupervision Engine configuration. Exiting.",
                         exit_code=0)
        ...
        if os.environ.get('ALIGNAK_USER'):
            ...
            print("FusionSupervision Engine user from system environment: %s" % self.user)
        if os.environ.get('ALIGNAK_GROUP'):
            self.group = os.environ.get('ALIGNAK_GROUP')
            print("FusionSupervision Engine group from system environment: %s" % self.group)
        ...
        # FusionSupervision Engine logger configuration file
        ...
        if self.logger_configuration != os.path.abspath(self.logger_configuration):
            if self.logger_configuration == './fusionsupervision-logger.json':
    ...
    def do_main_loop(self):
        ...
        """Main loop for an FusionSupervision Engine daemon
    ...
    def dump_environment(self):
        ...
        :return: None
        """
        # Dump the FusionSupervision Engine configuration to a temporary ini file
        path = os.path.join(tempfile.gettempdir(),
                            'dump-env-%s-%s-%d.ini' % (self.type, self.name, int(time.time())))

        try:
            with open(path, "w") as out_file:
                self.fusionsupervision_env.write(out_file)
        except Exception as exp:  # pylint: disable=broad-except
    ...
    def load_modules_manager(self):
        ...
        Note that this function is used by the FusionSupervision Engine modules. No self-use...
    ...
    def do_daemon_init_and_start(self, set_proc_title=True):
        ...
        The set_proc_title parameter is mainly useful for the FusionSupervision Engine unit tests.
        ...
        # try:
        #     for log_file in ['fusionsupervision.log', 'fusionsupervision-events.log']:
    ...
    def setup_communication_daemon(self):
        ...
        for incoming HTTP requests from other FusionSupervision Engine daemons
    ...
    def set_proctitle(self, daemon_name=None):
        ...
        if daemon_name:
            setproctitle("fusionsupervision-%s %s" % (self.type, daemon_name))
            if self.modules_manager:
                self.modules_manager.set_daemon_name(daemon_name)
        else:
            setproctitle("fusionsupervision-%s" % self.type)
    ...
    def get_header(self, configuration=False):
        ...
        """
        header = ["-----",
                  "FusionSupervision Engine %s - %s daemon" % (VERSION, self.name),
                  "Copyright (c) 2015-2018: FusionSupervision Team",
                  "License: AGPL",
                  "-----",
                  "My pid: %s" % self.pid]
    ...
    def hook_point(self, hook_name, handle=None):
        ...
        :type: handle: fusionsupervision.Satellite
        :return: None
        """
    ...
    def get_retention_data(self):  # pylint: disable=no-self-use
        ...
        :return: A list of FusionSupervision Engine object (scheduling items)
        :rtype: list
        """
        return []
    ...
    def get_id(self, details=False):  # pylint: disable=unused-argument
        ...
        ::
            {
                "fusionsupervision": selfFusionSupervision Engine instance name
                "type": daemon type
                "name": daemon name
                "version": FusionSupervision Engine version
            }
        ...
        # Modules information
        res = {
            "fusionsupervision": getattr(self, 'fusionsupervision_name', 'unknown'),
    ...
    def exit_on_error(self, message, exit_code=1):
        ...
        log += "-----\n"
        log += "You can get help at https://github.com/fusionsupervision/fusionsupervision-engine\n"
    ...
    def setup_fusionsupervision_logger(self):
        """ Setup fusionsupervision logger:
        - with the daemon log configuration properties
        - configure the global daemon handler (root logger)
        - log the daemon FusionSupervision Engine header

        - configure the global FusionSupervision Engine monitoring log
        ...
        logger.debug("FusionSupervision Engine daemon logger configured")
