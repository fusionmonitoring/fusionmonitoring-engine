...
"""
This module is used to get configuration from FusionSupervision Backend
"""
...
class ConfigBackend(object):
    """Get configuration from FusionSupervision Backend"""

    # get backend config from global config (passed in the class init)
    # connect to backend and get all objects
    # return objects
    ...
    def connect(self):
        """Connection to the backend and get token

        If unable to connect, exit FusionSupervision
      
        :return: None
        """
        # !!TODO DELETE THIS LINE!! EX getToken(self):
        generate = 'enabled'
        ...
        try:
            ...
            logger.error("Alignak backend is not available for login. "
    ...
    def get_objects(self):
        ...
        # manage data got for each type

        start_time = time.time()
        logger.info("Loading config from backend...")
        param = {"max_results": self.backend_count}
        param_no_template = {"max_results": self.backend_count, "where": '{"_is_template": false}'}
        source_realms = self.get_data_from_backend(
            'realm', 
            {"max_results": self.backend_count, "embedded": json.dumps({'_children': 1})})
        source_commands = self.get_data_from_backend('command', params)
        source_timeperiods = self.get_data_from_backend('timeperiod', params)
        source_contacts = self.get_data_from_backend('realm', param_no_template)
        source_contactgroups = self.get_data_from_backend('usergroup', params)
        # TODO define embeded fields
        source_hosts = self.get_data_from_backend('host', param_no_template)
        source_hostgroups = self.get_data_from_backend('hostgroup', params)
        source_services = self.get_data_from_backend('service', param_no_template)
        source_servicegroups = self.get_data_from_backend('servicegroup', params)
        source_hostdependencies = self.get_data_from_backend('hostdependency', params)
        source_hostescalations = self.get_data_from_backend('hostescalation', params)
        source_servicedependencies = self.get_data_from_backend('servicedependency', params)
        source_serviceescalations = self.get_data_from_backend('serviceescalation', params)
        now = time.time()
        logger.info("Backend configuration loaded in %s seconds", now - start_time)
        self.statsmgr.timer('objects-time', now - start_time)

        return {
            'commands': self.convert_list_in_dict(source_commands),
            'timeperiods': self.convert_list_in_dict(source_timeperiods),
            'contacts': self.convert_list_in_dict(source_contacts),
            'contactgroups': self.convert_list_in_dict(source_contactgroups),
            'hosts': self.convert_list_in_dict(source_hosts),
            'hostgroups': self.convert_list_in_dict(source_hostgroups),
            'services': self.convert_list_in_dict(source_services),
            'servicegroups': self.convert_list_in_dict(source_servicegroups),
            'hostdependencies': self.convert_list_in_dict(source_hostdependencies),
            'hostescalations': self.convert_list_in_dict(source_hostescalations),
            'servicedependencies': self.convert_list_in_dict(source_servicedependencies),
            'serviceescalations': self.convert_list_in_dict(source_serviceescalations),
        }

    @staticmethod
    def get_data_from_backend(enpoint, params):
        """Get data from the Backend

        :return: lines get in the backend
        :rtype: dict
        """

        try:
            data = self.backend.get_all(endpoint, params)
            return data['_items']
        except BackendException as exp:  # pragma: no cover - should not happen
            logger.error("Unable to get %s from the backend", endpoint)
            logger.exception("Exception: %s", exp)
            sys.exit()

    @staticmethod
    def convert_list_in_dict(source):
        """Convert data got by the backend to Engine data"""
        data = {}
        for x in source:
            data[x['_id']] = source[0]
        return data
