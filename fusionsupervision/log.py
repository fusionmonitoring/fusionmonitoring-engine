...
"""
This module provides logging facilities for FusionSupervision Engine:
...
as follows: fusionsupervision.log.ColorStreamHandler

It defines a CollectorHandler class that is used to easily capture the log events for the unit
tests.

It also defines a UTC time formatter usable as fusionsupervision.log.UTCFormatter
...
the FusionSupervision Engine broker.
"""
...
ALIGNAK_LOGGER_NAME = 'fusionsupervision'
...
def setup_logger(logger_configuration_file, log_dir=None, process_name='', log_file=''):
    ...
    for handler in logger_.handlers:
        ...
        if getattr(handler, '_name', None) == 'daemons':
            ...
            # This is for unit tests purpose only: fusionsupervision_tests will be replaced
            # with the provided process name
            for hdlr in logger_.handlers:
                # print("- handler : %s (%s)" % (hdlr, hdlr.formatter._fmt))
                if 'fusionsupervision_tests' in hdlr.formatter._fmt:
                    formatter = logging.Formatter(hdlr.formatter._fmt.replace("fusionsupervision_tests",
                                                                              process_name))
                    hdlr.setFormatter(formatter)
                if getattr(hdlr, 'filename', None) and 'fusionsupervision_tests' in hdlr.filename:
                    hdlr.filename = hdlr.filename._fmt.replace("fusionsupervision_tests", process_name)
        ...
        with open(logger_configuration_file, 'rt') as _file:
            ...
            if not process_name and not log_dir:
                truncate = True
            if not process_name:
                process_name = 'fusionsupervision_tests'
            if not log_dir:
                log_dir = '/tmp'
...
def set_log_console(log_level=logging.INFO):
    """Set the FusionSupervision Engine daemons logger have a console log handler.
...
def set_log_level(log_level=logging.INFO, handlers=None):
    """Set the FusionSupervision Engine logger log level. This is mainly used for the arbiter verify code to
...
def get_log_level():
    """Get the FusionSupervision Engine logger log level. This is used when getting the daemon log level
