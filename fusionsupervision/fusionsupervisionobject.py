...
""" This module contains only a common class for all object created in FusionSupervision Engine: FusionsupervisionObject.
"""

import uuid
from copy import copy
from fusionsupervision.property import NONE_OBJECT, SetProp


def get_a_new_object_id():
    """
    Get a new FusionSupervision Engine object identifier. Uses the uuid version 1 generator
...
class FusionsupervisionObject(object):
    """This class provides a generic way to instantiate fusionsupervision objects.
    ...
    def __init__(self, params=None, parsing=True):  # pylint: disable=unused-argument
        ...
        read by the FusionSupervision Engine arbiter else the objects are restored from a previously
        ...
        if parsing:
            ...
            # else:
            #     print("FusionsupervisionObject: parsing but already have an uuid! Parameters: %s" % params)
            return

        self.fill_default()
        if params is None:
            # Object is created without any parameters
            # print("FusionsupervisionObject: initialized with no parameters but default properties!")
            return

        if 'uuid' not in params:
            # print("FusionsupervisionObject: no parsing but do not provide an uuid! Parameters: %s" % params)
            self.uuid = get_a_new_object_id()
