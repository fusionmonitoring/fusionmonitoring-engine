...
from fusionsupervision.log import make_monitoring_log, set_log_level, set_log_console
from fusionsupervision.misc.common import SIGNALS_TO_NAMES_DICT
from fusionsupervision.misc.serialization import unserialize, FusionsupervisionClassLookupException
from fusionsupervision.objects.config import Config
from fusionsupervision.macroresolver import MacroResolver
from fusionsupervision.external_command import ExternalCommandManager
from fusionsupervision.dispatcher import Dispatcher
from fusionsupervision.daemon import Daemon
from fusionsupervision.stats import statsmgr
...
from fusionsupervision.http.arbiter_interface import ArbiterInterface
from fusionsupervision.objects.satellitelink import SatelliteLink
from fusionsupervision.monitor import MonitorConnection
...
class Arbiter(Daemon):  # pylint: disable=R0902
    ...
    def __init__(self, **kwargs):
        ...
        if 'legacy_cfg_files' in kwargs and kwargs['legacy_cfg_files']:
            logger.warning(
                ...
                "parameter to provide a global FusionSupervision Engine configuration file. "
                "** Note that this feature is not removed because it is still used "
                "for the unit tests of the FusionSupervision Engine! If some monitoring files are "
        ...
        #     sys.exit("The FusionSupervision Engine environment file is not existing "
        ...
        self.fusionsupervision_name = self.name
        if 'fusionsupervision_name' in kwargs and kwargs['fusionsupervision_name']:
            self.fusionsupervision_name = kwargs['fusionsupervision_name']
        self.arbiter_name = self.fusionsupervision_name

        # Dump system health, defaults to report every 5 loop count
        self.system_health = False
        self.system_health_period = 5
        if 'FUSIONSUPERVISION_SYSTEM_MONITORING' in os.environ:
            self.system_health = True
            try:
                self.system_health_period = int(os.environ.get('FUSIONSUPERVISION_SYSTEM_MONITORING', '5'))
    ...
    def load_monitoring_config_file(self, clean=True):
        ...
        """Load main configuration file (fusionsupervision.cfg)::
        ...
        * Cut conf into parts and prepare it for sending
        ...
        fusionsupervision-backend-import script) may need to avoid the cleaning ;)
        ...
        if self.verify_only:
            # Force adding a console handler to the FusionSupervision logger
        ...
        # if not self.fusionsupervision_env:
        #     self.exit_on_error("*** No FusionSupervision environment file. Exiting...", exit_code=2)
        # else:
        ...
        # FusionSupervision Engine global environment file
        # -------------------------------
        # Here we did not yet read the FusionSupervision Engine configuration file (except for the Arbiter daemon
        # configuration.
        # We must get the FusionSupervision Engine macros and global configuration parameters
        # ---------------------
        # Manage FusionSupervision Engine macros; this before loading the legacy configuration files
        # with their own potential macros
        # ---------------------
        macros = []
        # Get the macros / variables declared in the FusionSupervision Engine environment (fusionsupervision.ini) file!
        if self.fusionsupervision_env:
            # The properties defined in the fusionsupervision.cfg file are not yet set! So we set the one
            # got from the environment
            logger.info("Getting FusionSupervision Engine macros...")
            fusionsupervision_macros = self.fusionsupervision_env.get_fusionsupervision_macros()
            if fusionsupervision_macros:
                # Remove the leading and trailing underscores
                for key in sorted(fusionsupervision_macros.keys()):
                    value = fusionsupervision_macros[key]
                    if key[0] == '_' or key[0] == '$':
                        key = key[1:]
                    if key[-1] == '_' or key[-1] == '$':
                        key = key[:-1]
                    ...
                    logger.debug("- FusionSupervision Engine macro '$%s$' = %s", key.upper(), value)

            # and then the global configuration.
            # The properties defined in the fusionsupervision.cfg file are not yet set! So we set the one
            # got from the appropriate section of the FusionSupervision Engine environment file
            logger.info("Getting FusionSupervision Engine configuration...")
            fusionsupervision_configuration = self.fusionsupervision_env.get_fusionsupervision_configuration()
            if fusionsupervision_configuration:
                for key in sorted(fusionsupervision_configuration.keys()):
                    value = fusionsupervision_configuration[key]
                    if key.startswith('_'):
                ...
                logger.info("Got FusionSupervision Engine global configuration")

        self.fusionsupervision_name = getattr(self.conf, "fusionsupervision_name", self.name)
        logger.info("Configuration for FusionSupervision Engine: %s", self.fusionsupervision_name)

        if macros:
            self.conf.load_params(macros)

        # Here we got the macros and fusionsupervision configuration variables from the
        # fusionsupervision.ini configuration!
        # The self Config object is now initialized with the global FusionSupervision Engine variables.
        ...
        raw_objects = self.conf.read_config_buf(
            self.conf.read_legacy_cfg_files(self.legacy_cfg_files,
                                            self.fusionsupervision_env.cfg_files if self.fusionsupervision_env
                                            else None)
        )

        if self.fusionsupervision_name != getattr(self.conf, "fusionsupervision_name", self.name):
            self.fusionsupervision_name = getattr(self.conf, "fusionsupervision_name", self.name)
            logger.warning("FusionSupervision Engine name changed from the legacy Cfg files: %s", self.fusionsupervision_name)
        ...
        # declared in the FusionSupervision Engine environment (fusionsupervision.ini) file!
        if self.fusionsupervision_env:
            ...
            # Get all the FusionSupervision Engine daemons from the configuration
            logger.info("Getting daemons configuration...")
            some_daemons = False
            for daemon_name, daemon_cfg in list(self.fusionsupervision_env.get_daemons().items()):
            ...
            for daemon_type in ['arbiter', 'scheduler', 'broker',
                ...
                for cfg_daemon in raw_objects[daemon_type]:
                    ...
                    cfg_daemon['modules'] = \
                        self.fusionsupervision_env.get_modules(daemon_name=cfg_daemon['name'])
                    for module_daemon_type, module in extra_modules:
                        if module_daemon_type == daemon_type:
                            cfg_daemon['modules'].append(module['name'])
                            logger.info("- added an FusionSupervision Engine inner module '%s' to the %s: %s",
            ...
            if not some_legacy_daemons:
                ...
                logger.info("- some dameons are configured in legacy Cfg files. "
                            "You should update the configuration with the new FusionSupervision Engine "
                            "configuration file.")
            ...
            for _, module_cfg in list(self.fusionsupervision_env.get_modules().items()):
        ...
        params = {}
        if self.fusionsupervision_env:
            params = self.fusionsupervision_env.get_fusionsupervision_configuration()
        ...
        if not self.link_to_myself:
            ...
            self.request_stop("Error: I cannot find my own configuration (%s), I bail out. "
                              ...
                              "the FusionSupervision Engine configuration file (certainly fusionsupervision.ini) "
                              "with the value '%s'."
        ...
        # Perharps, loading only the fusionsupervision.ini would be enough for a spare arbiter.
        ...
        # Call modules get_fusionsupervision_configuration() to load FusionSupervision Engine configuration parameters
        ...
        # (example modules: fusionsupervision_backend)
        # _t0 = time.time()
        # self.load_modules_fusionsupervision_configuration()
        # statsmgr.timer('core.hook.get_fusionsupervision_configuration', time.time() - _t0)
        ...
        # Here we got all our FusionSupervision Engine configuration and the monitored system configuration
        ...
        # Dump FusionSupervision Engine macros
        logger.debug("FusionSupervision Engine global macros:")
        ...
        # REF: doc/fusionsupervision-conf-dispatching.png (2)
        ...
        if not self.daemons_start(run_daemons=False):
            self.conf.show_errors()
            self.request_stop("*** FusionSupervision Engine will not be able to manage the configured daemons. "
        ...
        if self.verify_only:
            ...
            if self.conf.missing_daemons:
                logger.warning("Some missing daemons were detected in the parsed configuration. "
                               ...
                               "else FusionSupervision Engine will use its default configuration.")
    ...
    def load_modules_configuration_objects(self, raw_objects):  # pragma: no cover,
        ...
        for instance in self.modules_manager.instances:
            ...
            try:
                logger.info("Getting FusionSupervision Engine monitored configuration objects from module '%s'",
                            instance.name)
    ...
    def load_modules_fusionsupervision_configuration(self):  # pragma: no cover, not yet with unit tests.
        """Load FusionSupervision Engine configuration from the arbiter modules
        If module implements get_fusionsupervision_configuration, call this function

        :param raw_objects: raw objects we got from reading config files
        :type raw_objects: dict
        :return: None
        """
        fusionsupervision_cfg = {}
        # Ask configured modules if they got configuration for us
        for instance in self.modules_manager.instances:
            if not hasattr(instance, 'get_fusionsupervision_configuration'):
                return

            try:
                logger.info("Getting FusionSupervision Engine global configuration from module '%s'", instance.name)
                cfg = instance.get_fusionsupervision_configuration()
                fusionsupervision_cfg.update(cfg)
            except Exception as exp:  # pylint: disable=broad-except
                logger.error("Module %s get_fusionsupervision_configuration raised an exception %s. "
        ...
        params = []
        if fusionsupervision_cfg:
            logger.info("Got FusionSupervision Engine global configuration:")
            for key, value in sorted(fusionsupervision_cfg.items()):
    ...
    def start_daemon(self, satellite):
        ...
        if not daemon_script_location:
            daemon_script_location = "fusionsupervision-%s" % satellite.type
        else:
            daemon_script_location = "%s/fusionsupervision-%s" % (daemon_script_location, satellite.type)

        # Some extra arguments may be defined in the FusionSupervision Engine configuration
    ...
    def daemons_start(self, run_daemons=True):
        ...
        if run_daemons:
            logger.info("FusionSupervision Engine configured daemons start:")
        else:
            logger.info("FusionSupervision Engine configured daemons check:")
        ...
        for satellites_list in [self.conf.arbiters, self.conf.receivers, self.conf.reactionners,
            ...
            for satellite in satellites_list:
                logger.info("- found %s, to be launched: %s, address: %s",
                            satellite.name, satellite.fusionsupervision_launched, satellite.uri)
                ...
                if satellite.address not in ['127.0.0.1', 'localhost']:
                    logger.error("FusionSupervision Engine is required to launch a daemon for %s %s "
                ...
                if not satellite.fusionsupervision_launched:
                    logger.debug("FusionSupervision Engine will not launch '%s'")
                    continue
    ...
    def daemons_check(self):
        """Manage the list of FusionSupervision Engine launched daemons
        ...
        logger.debug("FusionSupervision Engine launched daemons check")
        result = True
    ...
    def daemons_stop(self, timeout=30, kill_children=False):
        """Stop the FusionSupervision Engine daemons
        ...
        if self.my_daemons:
            logger.info("FusionSupervision Engine self-launched daemons stop:")
    ...
    def daemons_reachability_check(self):
        ...
        _t0 = time.time()
        logger.debug("FusionSupervision Engine daemons reachability check")
        result = self.dispatcher.check_reachable()
        statsmgr.timer('dispatcher.check-alive', time.time() - _t0)

        _t0 = time.time()
        logger.debug("FusionSupervision Engine daemons status get")
        ...
        # Send the collected events to the FusionSupervision Engine logger
        for event in events:
            event.prepare()
    ...
    def setup_new_conf(self):
        ...
        with self.conf_lock:
            ...
            # Update FusionSupervision Engine name
            self.fusionsupervision_name = self.cur_conf['fusionsupervision_name']
            logger.info("My FusionSupervision Engine instance: %s", self.fusionsupervision_name)
    ...
    def configuration_dispatch(self, not_configured=None):
        ...
        if not not_configured:
            ...
            # Without a correct configuration, FusionSupervision Engine daemons will not run correctly
    ...
    def do_before_loop(self):
        ...
        if not self.daemons_start(run_daemons=True):
            self.request_stop(message="Some FusionSupervision Engine daemons did not started correctly.",
                              exit_code=4)

        if not self.daemons_check():
            self.request_stop(message="Some FusionSupervision Engine daemons cannot be checked.",
                              exit_code=4)
    ...
    def do_loop_turn(self):
        ...
        * Push broks and external commands to the satellites
        ...
        if self.loop_count % self.fusionsupervision_monitor_period == 1:
            self.get_fusionsupervision_status(details=True)

        # Maybe an external process requested FusionSupervision Engine stop...
    ...
    def get_fusionsupervision_status(self, details=False):
        # pylint: disable=too-many-locals, too-many-branches
        """Push the fusionsupervision overall state as a passive check

        Build all the daemons overall state as a passive check that can be notified
        to the FusionSupervision Engine WS

        The FusionSupervision Engine Arbiter is considered as an host which services are all the FusionSupervision Engine
        running daemons. An FusionSupervision Engine daemon is considered as a service of an FusionSupervision Engine host.

        As such, it reports its status as a passive service check formatted as defined for
        the FusionSupervision Engine WS module (see http://fusionsupervision-module-ws.readthedocs.io)
        ...
        res = {
            "name": inner_stats['fusionsupervision'],
            "template": {
                "_templates": ["fusionsupervision", "important"],
                "alias": inner_stats['fusionsupervision'],
        ...
        # Create self arbiter service - I am now considered as a service for my FusionSupervision Engine monitor!
        ...
        # FusionSupervision Engine performance data are:
        ...
        if 'daemons_states' in inner_stats:
            ...
            if state == 2:  # UNREACHABLE
                log_level = 'warning'
            if self.conf.log_fusionsupervision_checks or state > 0:
                self.add(make_monitoring_log(log_level, 'ALIGNAK CHECK;%s;%d;%s;%s' % (
                    self.fusionsupervision_name, state, res['livestate']['output'],
        ...
        if self.fusionsupervision_monitor:
            logger.debug("Pushing FusionSupervision Engine passive check to %s: %s", self.fusionsupervision_monitor, res)

            if self.my_monitor is None:
                self.my_monitor = MonitorConnection(self.fusionsupervision_monitor)

            if not self.my_monitor.authenticated:
                self.my_monitor.login(self.fusionsupervision_monitor_username,
                                      self.fusionsupervision_monitor_password)

            result = self.my_monitor.patch('host', res)
            logger.debug("Monitor reporting result: %s", result)
        else:
            logger.debug("No configured FusionSupervision Engine monitor to receive: %s", res)
    ...
    def main(self):
        ...
        * Endless main process loop
        ...
        try:
            ...
            if self.verify_only:
                self.setup_fusionsupervision_logger()
